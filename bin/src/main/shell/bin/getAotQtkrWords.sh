...
CONF_SH=../conf/getAotQtkrWords.conf
[ -f "${CONF_SH}" ] && source $CONF_SH || echo "not exist ${CONF_SH} "
...
#params in alert.sh 
type="ERROR"
module=getAotQtkrWords
program=getAotQtkrWords.sh
reader_list=zhangzhenhua02

mkdir -p ${LOG_PATH}
mkdir -p ${DATA_PATH}
mkdir -p ${BAK_PATH}

#LOG_FILE=${LOG_PATH}/getAotQtkrWords.log
LOG_FILE_1=${LOG_PATH}/getAotQtkrWordsLog/java.log
LOG_FILE_2=${LOG_PATH}/getAotQtkrWordsLog/create.log
LOG_FILE_3=${LOG_PATH}/getAotQtkrWordsLog/mongo.log
LOG_FILE_4=${LOG_PATH}/getAotQtkrWordsLog/rename.log
...
function fetch_aot_qtkrword()
{
	...
	java -Xms2048m -Xmx4096m -classpath ${CUR_CLASSPATH} com.baidu.beidou.aot.ImportAotQtkrWord ${DATA_PATH}${QTKRWORD_NAME} ${DATA_PATH}${QTKRWORD_RESULT_NAME}.${date_download} 0 30  > ${LOG_FILE_1} 2>&1
	alert $? "计算QT主动推荐失败"
}
...
function loadDataToMongo(){
	...
	server="$1"
        ...
        ${MONGO_CLIENT} ${host}:${port}/${AOTQTKR_MONGO_DB} -u${MONGO_DB_USER} -p${MONGO_DB_PASSWORD} create.js > ${LOG_FILE_2} 2>&1
	if [ $? -ne 0 ] ; then
		return 1;
	fi
        ...
        ${MONGOIMPORT_CLIENT} -h${host} --port ${port} -u${MONGO_DB_USER} -p${MONGO_DB_PASSWORD} -d${AOTQTKR_MONGO_DB} -c${AOTQTKR_MONGO_COLLECTION}_tmp -f groupid,wordid  --type "tsv" --file ${DATA_PATH}/${QTKRWORD_RESULT_NAME}.${date_download} > ${LOG_FILE_3} 2>&1 &
	for pid in `jobs -p`;do
        ...
        ${MONGO_CLIENT} ${host}:${port}/${AOTQTKR_MONGO_DB} -u${MONGO_DB_USER} -p${MONGO_DB_PASSWORD} rename.js > ${LOG_FILE_4} 2>&1
	if [ $? -ne 0 ] ; then
		return 1;
	fi
