...
#@data:2012-07-12
#@version: 1.0.0.0
#@brief:download and mapping targetType clks file

CONF=../conf/get_targettype_clks.conf
...
program=get_targettype_clks.sh
...
function wgetFiles(){
	...
	if [ $? -ne 0 ]
	then
	   echo "`date +"%Y-%m-%d %H:%M:%S"`,get_targettype_clks download ${TARGETTYPE_FILE_NAME}.${DATE_TO_DOWNLOAD} failed">>${LOG_FILE}
	...
	if [ $? -ne 0 ]
	then
	   echo "`date +"%Y-%m-%d %H:%M:%S"`,get_targettype_clks download ${TARGETTYPE_FILE_NAME}.${DATE_TO_DOWNLOAD}.md5 failed">>${LOG_FILE}
	...
	if [ $? -ne 0 ]
	then
		echo "`date +"%Y-%m-%d %H:%M:%S"`,get_targettype_clks check ${TARGETTYPE_FILE_NAME}.${DATE_TO_DOWNLOAD}.md5 failed">>${LOG_FILE}
...
function checkResult(){
    ...
    then
		alert 1 "generate targettype_clks file for ${DATE_TO_DOWNLOAD} failed."
	    return 1
    fi
}
...
function main(){
	...
	startTime=`date +"%s"`
	echo "`date +"%Y-%m-%d %H:%M:%S"`,get_targettype_clks start run.">>${LOG_FILE}
	...
	echo "`date +"%Y-%m-%d %H:%M:%S"`,get_targettype_clks run success, consume ${spendTime}s.">>${LOG_FILE}
}
