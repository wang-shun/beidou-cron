	...
	awk -F'\t' -v v=',' -v m=500 'ARGIND==1{tradeMap[$1]=$2}ARGIND==2{key=$2""$3;
		...
		for(i in map){
			if(map[i] != ""){
			  ...
			  print "UPDATE cprounitmater"table" m, cprounitstate"table" s SET m.new_adtradeid="substr(i,0,6)", m.adtradeid="oldtradeid", m.confidence_level="substr(i,7,1)"\
			  	, m.tag_mask=(m.tag_mask&1073741816)|"substr(i,7,1)" WHERE m.id IN ("map[i]") AND m.id = s.id AND s.helpstatus & 2 = 0 ;\
			  	update cprounitmater"table" m, online_unit o, cprounitstate"table" s \
			  	 set o.new_adtradeid=m.new_adtradeid, o.adtradeid=m.adtradeid, o.tag_mask=m.tag_mask, o.confidence_level=m.confidence_level \
			  	 where m.id=o.id and m.id=s.id and s.state=0 and o.new_adtradeid != m.new_adtradeid and s.id IN ("map[i]");";
			}
		}
	}' ${NEWTRADEID_OLDTRADEID_FILE} ${shard_table_file} > ${shard_table_file}.sql
	
	local tempsqlfile=importtempfile.sql.${idx}
	local count=0
