...
[ -f "${CONF_SH}" ] && source $CONF_SH || echo "not exist ${CONF_SH} "
CONF_SH="../conf/importTradeWord.conf"
[ -f "${CONF_SH}" ] && source $CONF_SH || echo "not exist ${CONF_SH} "
CONF_SH="../lib/db_sharding.sh"
[ -f "${CONF_SH}" ] && source $CONF_SH || echo "not exist ${CONF_SH}"
CONF_SH=alert.sh
[ -f "${CONF_SH}" ] && source $CONF_SH || echo "not exist ${CONF_SH} "
program=importTradeWord.sh
...
LOG_FILE=${LOG_PATH}/importTradeWord.log
...
function fetch_data_from_db()
{
	...
	do
		...
		#按照点击、展现排序
		sort -k4nr,4 -k5nr,5 -t, ${WORD_FILE_PATH}.$index.total > ${WORD_FILE_PATH}.$index.sort
