...
#@file:importWordStatInfo.sh
#@author:wangchongjie
#@date:2012-05-17
#@version:1.0.0.0
#@brief:将log平台统计的KT结果:日军展现量（含高级）导入mongodb
...
[ -f "${CONF_SH}" ] && source $CONF_SH || echo "not exist ${CONF_SH} "
CONF_SH="../conf/importWordStatInfo.conf"
[ -f "${CONF_SH}" ] && source $CONF_SH || echo "not exist ${CONF_SH} "

CONF_SH=alert.sh
[ -f "${CONF_SH}" ] && source $CONF_SH || echo "not exist ${CONF_SH} "
program=importWordStatInfo.sh

LOG_FILE=${LOG_PATH}/importWordStatInfo.log
