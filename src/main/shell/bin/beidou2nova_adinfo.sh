...
function mergeValidData()
{
	...
	rm -f ${LOCAL_EFFECT_USER_FILTER}
	rm -f ${LOCAL_EFFECT_PLAN_FILTER}
	rm -f ${LOCAL_EFFECT_GROUP_FILTER}
	rm -f ${LOCAL_EFFECT_UNIT_FILTER}
...
function exportUserVipInfo()
{
	...
	rm -f ${ULEVEL_FILE_NAME}*
    wget -q ftp://${ULEVEL_FILE_FTP_NAME}:${ULEVEL_FILE_FTP_PWD}@${ULEVEL_FILE_SERVER}/${ULEVEL_FILE_PATH}/${ULEVEL_FILE_NAME}
	if [ $? -ne 0 ];then
       PRINT_LOG "下载shifen的用户文件失败"
       return 1
    fi
	wget -q ftp://${ULEVEL_FILE_FTP_NAME}:${ULEVEL_FILE_FTP_PWD}@${ULEVEL_FILE_SERVER}/${ULEVEL_FILE_PATH}/${ULEVEL_FILE_NAME}.md5
	if [ $? -ne 0 ];then
       PRINT_LOG "下载shifen的用户文件MD5失败"
       return 1
    fi
    ULEVEL_MD5=`md5sum ${ULEVEL_FILE_NAME}`
    if [ "${ULEVEL_MD5}" != "`cat ${ULEVEL_FILE_NAME}.md5`" ] ;then
	...
	#生成ulevel文件
	rm -f ${UVIP_FILE}
	awk -F"\t" 'BEGIN{OFS="\t"} {if($6==10101){print $3,$1,0} else if($6==10104) {print $3,$1,1}}' ${ULEVEL_FILE_NAME} > ${UVIP_FILE}
	if [ $? -ne 0 ];then
...
if [ $1 -eq 1 ]; then
    ...
    PRINT_LOG "export  effect ad info data"
    op_with_retry "mergeValidData" "export effect ad info data failed" $MAX_RETRY
	
fi



#param 2, prepare effect data，export effect budget
if [ $1 -eq 2 ]; then
    PRINT_LOG "import search data and export effect ad info"
    op_with_retry "mergeValidData" "export effect ad info" $MAX_RETRY
    PRINT_LOG "export valid plan data"
    op_with_retry "mergeValidPlanData" "export valid plan data failed" $MAX_RETRY
	PRINT_LOG "export ValidBudgetFiles"
	op_with_retry "exportValidBudgetFiles" "export ValidBudgetFiles failed" $MAX_RETRY
