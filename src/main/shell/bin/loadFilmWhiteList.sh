...
program=loadFilmWhiteList.sh
reader_list=yang_yun

LOG_FILE=${LOG_PATH}/loadFilmWhiteList.log
...
msg="执行远程调用-载入可投放贴片广告的用户白名单失败("$1")"

url=http://$1:8080/rpc/loadFilmWhiteList
...
function call_noport() {
    msg="执行远程调用-载入可投放贴片广告的用户白名单失败("$1")"
    url=http://$1/rpc/loadFilmWhiteList
    java -cp ${CUR_CLASSPATH} com.baidu.ctclient.HessianRpcClientUsingErrorCode $url $username $password
    alert_return $? "${msg}"
}
