...
function backup_subtable
{
	...
	awk -vmth="$month" -vidx="$index" -vmax="$MAX_PER_PAGE" '
		...
		{if(NR==2)
		{
			print "insert into operationtext_bak_"mth" select * from operationtext"idx" where id in (0";
	...
	runsql_xdb "use history; `cat ${EXPORT_PATH}${OUTPUT_FILE}${index}.importoptxt.sql`"
	if [ $? -ne 0 ]
	then
...
function clear_subtable
{
	...
	echo "delete from operationhistory$index where id in (" > ${EXPORT_PATH}${OUTPUT_FILE}${index}.clearophis.sql
	...
	runsql_xdb "use history; `cat ${EXPORT_PATH}${OUTPUT_FILE}${index}.clearophis.sql`"
	if [ $? -ne 0 ]
	then
	...
	echo "delete from operationtext$index where id in (0" > ${EXPORT_PATH}${OUTPUT_FILE}${index}.clearoptxt.sql
	...
	runsql_xdb "use history; `cat ${EXPORT_PATH}${OUTPUT_FILE}${index}.clearoptxt.sql`"
	if [ $? -ne 0 ]
	then
