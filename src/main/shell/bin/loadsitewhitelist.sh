...
program=loadsitewhitelist.sh
reader_list=zhuqian

LOG_FILE=${LOG_PATH}/loadsitewhitelist.log
...
msg="执行远程调用-载入百度自有流量网站失败("$1")"

url=http://$1:8080/rpc/loadSiteWhiteList
...
function call_noport() {
    msg="执行远程调用-载入百度自有流量网站失败("$1")"
    url=http://$1/rpc/loadSiteWhiteList
    java -cp ${CUR_CLASSPATH} com.baidu.ctclient.HessianRpcClientUsingErrorCode $url $username $password    
    alert_return $? "${msg}"
}
