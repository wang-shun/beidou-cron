...
#8库数据分别读到8个数据文件
sql="select userid,groupid from beidou.cprogroup where targettype=8 and [userid]" 
for (( i=0;i<8;i++))
do
...
do
	...
	do
		...
		echo "update beidou.cprogroup set targettype =8 where groupid = "$groupid";" >> $ROLLBACK_SQL_DATA
		where=$where","$groupid
		let count++
		if [ $count -eq 1000 ];then
			echo "update beidou.cprogroup set targettype = 16,modtime=now() where groupid in "$where");" >> $CPROGROUP_DATA_PATH_OUTPUT"/update_sql"$i$i
			where="(0"
			count=0
		fi
