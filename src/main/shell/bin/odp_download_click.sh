...
#@file: odp_download_click.sh
#@author: wangchongjie
#@date: 2012-07-12
#@version: 3.0.0
#@brief: click data download & split 

DEBUG_MOD=0
#WORK_PATH="/home/work/beidou-cron"

CONF_FILE="./conf/odp_download_click.conf"
...
function trans_holmes_hour_null()
{
	local curTime=$1
	local alertType=$2
	local curDate=${curTime:0:8}

	local localDestPath=${LOCAL_DEST}/normal/${curDate}
	
	if ! [ -e $LOCAL_DEST/normal/$curDate ]
	then
        mkdir -p $LOCAL_DEST/normal/$curDate
	fi

	touch ${localDestPath}/${TRANS_PLAN_FILE_PRE}${curTime}${FILE_SUF}
	touch ${localDestPath}/${TRANS_GROUP_FILE_PRE}${curTime}${FILE_SUF}
	touch ${localDestPath}/${TRANS_AD_FILE_PRE}${curTime}${FILE_SUF}
	touch ${localDestPath}/${TRANS_SITE_FILE_PRE}${curTime}${FILE_SUF}
	touch ${localDestPath}/${TRANS_USER_FILE_PRE}${curTime}${FILE_SUF}
	touch ${localDestPath}/${TRANS_TRADE_FILE_PRE}${curTime}${FILE_SUF}
	touch ${localDestPath}/${TRANS_REG_FILE_PRE}${curTime}${FILE_SUF}
	touch ${localDestPath}/${TRANS_PACK_FILE_PRE}${curTime}${FILE_SUF}
	touch ${localDestPath}/${TRANS_KEYWORD_FILE_PRE}${curTime}${FILE_SUF}
	touch ${localDestPath}/${TRANS_IT_FILE_PRE}${curTime}${FILE_SUF}
	touch ${localDestPath}/${TRANS_DT_FILE_PRE}${curTime}${FILE_SUF}
	touch ${localDestPath}/${TRANS_AT_FILE_PRE}${curTime}${FILE_SUF}
	touch ${localDestPath}/${HOLMES_FILE_PRE}${curTime}${FILE_SUF}
	
	cd ${localDestPath}
	md5sum ${PLAN_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${PLAN_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${GROUP_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${GROUP_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${AD_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${AD_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${GROUP_SITE_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${GROUP_SITE_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${TRANS_PLAN_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${TRANS_PLAN_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${TRANS_GROUP_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${TRANS_GROUP_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${TRANS_AD_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${TRANS_AD_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${TRANS_SITE_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${TRANS_SITE_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${TRANS_USER_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${TRANS_USER_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${TRANS_TRADE_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${TRANS_TRADE_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${TRANS_REG_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${TRANS_REG_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${TRANS_PACK_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${TRANS_PACK_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${TRANS_KEYWORD_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${TRANS_KEYWORD_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${TRANS_IT_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${TRANS_IT_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${TRANS_DT_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${TRANS_DT_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	md5sum ${TRANS_AT_FILE_PRE}${curTime}${FILE_SUF} > "${localDestPath}/${TRANS_AT_FILE_PRE}${curTime}${FILE_SUF}"".md5"
	
	md5sum ${HOLMES_FILE_PRE}${curTime}${FILE_SUF} > "${HOLMES_FILE_PRE}${curTime}${FILE_SUF}"".md5"
}
...
function download_dayfile_hour()
{
	...
	trans_holmes_hour_null $curTime $alertType
	...
	#deal with anti stat day ------ begin
	statFileName=${STAT_ANTI_DAY_FILE_PRE}${curTime}${FILE_SUF}
	localStatTempFile=$localDestPath/$statFileName
	localStatDestFile=$localDestPath/$statFileName
	
	if [ $STAT_ANTI_SWITCH -eq 1 ]
	then
		if [ -e $DAYFILE_SUCCESS_PATH/${STAT_ANTI_DAY_FILE_PRE}$curDate".isSuccess" ]  
        then
			touch $localStatTempFile
		elif [ ${curDate} -eq `date   +%Y%m%d` ] && [ ${curHour} -lt ${ANTI_STAT_DAY_EXE_TIME} ] 
		then 
			touch $localStatTempFile
		else 
			download_stat_hour $STAT_ANTI_DAY_FILETYPE $alertType "kun"
			if [ $? -ne 0 ]
			then
				SendMailAndMessage "odp_beidou_srchs: Fail to download [$STAT_ANTI_DAY_FILETYPE]." $alertType
				return 1
			fi
			touch $DAYFILE_SUCCESS_PATH/${STAT_ANTI_DAY_FILE_PRE}$curDate".isSuccess"
		fi
	else
		#if STAT_ANTI_SWITCH is off, then touch empty file
		cat /dev/null > $statFileName
		md5sum $statFileName > $localStatTempFile".md5"
		log "WARN" "If uv_device switch is off, then touch empty file[$localStatTempFile]!"
	fi	
...
#download click stat data of one hour
#$1: YYYYmmddHH
#$2: alertType[0:send both mail and message; 1:only send mail; 2:only send message; 3:no mail or message]
function download_beidou_hour()
{
	...
	if [ $? -ne 0 ]
	then
		SendMailAndMessage "odp_beidou_click: Fail to download [$statFileName]." $alertType
		return 1
	fi		
	md5sum $statFileName > $localStatTempFile".md5"
	
	#deal with it click stat ------ begin
	statFileName=${ITSTAT_FILE_PRE}${curTime}${FILE_SUF}
	localStatTempFile=$LOCAL_TEMP/$statFileName
	localStatDestFile=$localDestPath/$statFileName
	
	if [ $ITSTAT_CLICK_SWITCH -eq 1 ]
	then
		...
		#if ITSTAT_CLICK_SWITCH is off, then touch empty file, and record log, and go on
		cat /dev/null > $statFileName
		md5sum $statFileName > $localStatTempFile".md5"
		log "WARN" "If it click stat switch is off, then touch empty file[$localStatTempFile]!"
	fi
	#deal with it cllick stat ------ end
	
	#deal with dt click stat ------ begin
	statFileName=${DTSTAT_FILE_PRE}${curTime}${FILE_SUF}
	localStatTempFile=$LOCAL_TEMP/$statFileName
	localStatDestFile=$localDestPath/$statFileName
	
	if [ $DTSTAT_CLICK_SWITCH -eq 1 ]
	then
		...
		#if DTSTAT_CLICK_SWITCH is off, then touch empty file, and record log, and go on
		cat /dev/null > $statFileName
		md5sum $statFileName > $localStatTempFile".md5"
		log "WARN" "If dt click stat switch is off, then touch empty file[$localStatTempFile]!"
	fi
	#deal with dt click stat ------ end
	
	#deal with new kt click stat ------ begin
	statFileName=${KEYWORD_FILE_PRE}${curTime}${FILE_SUF}
	localStatTempFile=$LOCAL_TEMP/$statFileName
	localStatDestFile=$localDestPath/$statFileName
	
	if [ $KEYWORD_CLICK_SWITCH -eq 1 ]
	then
		...
		#if KEYWORD_CLICK_SWITCH is off, then touch empty file, and record log, and go on
		cat /dev/null > $statFileName
		md5sum $statFileName > $localStatTempFile".md5"
		log "WARN" "If new kt click stat switch is off, then touch empty file[$localStatTempFile]!"
	fi
	#deal with new kt click stat ------ end
	...
	then
		...
		cat /dev/null > $statFileName
		md5sum $statFileName > $localStatTempFile".md5"
		log "WARN" "If device srchs stat switch is off, then touch empty file[$localStatTempFile]!"
	fi
	#deal with device srchs stat ------ end
	
	#deal with tax srchs stat ------ begin
	statFileName=${STAT_TAX_FILE_PRE}${curTime}${FILE_SUF}
	localStatTempFile=$LOCAL_TEMP/$statFileName
	localStatDestFile=$localDestPath/$statFileName
	
	if [ $STAT_TAX_SWITCH -eq 1 ]
	then
		download_stat_hour $STAT_TAX_FILETYPE $alertType "kun"
		if [ $? -ne 0 ]
		then
			SendMailAndMessage "odp_beidou_srchs: Fail to download [$statFileName]." $alertType 
			return 1
		...
		#if SWITCH is off, then touch empty file, and record log, and go on
		cat /dev/null > $statFileName
		md5sum $statFileName > $localStatTempFile".md5"
		log "WARN" "If tax srchs stat switch is off, then touch empty file[$localStatTempFile]!"
	fi
	#deal with tax srchs stat ------ end
	
	#deal with anti srchs stat ------ begin
	...
	if [ $STAT_ANTI_SWITCH -eq 1 ]
	then
		download_stat_hour $STAT_ANTI_FILETYPE $alertType "kun"
		if [ $? -ne 0 ]
		then
			SendMailAndMessage "odp_beidou_srchs: Fail to download [$statFileName]." $alertType 
			return 1
		...
		cat /dev/null > $statFileName
		md5sum $statFileName > $localStatTempFile".md5"
		log "WARN" "If anti srchs stat switch is off, then touch empty file[$localStatTempFile]!"
	fi
	#deal with anti srchs stat ------ end
	
	#deal with si keyword click stat ------ begin
	statFileName=${SI_KEYWORD_FILE_PRE}${curTime}${FILE_SUF}
	localStatTempFile=$LOCAL_TEMP/$statFileName
	localStatDestFile=$localDestPath/$statFileName
	
	if [ $SI_KEYWORD_CLICK_SWITCH -eq 1 ]
	then
		...
		#if SI_KEYWORD_CLICK_SWITCH is off, then touch empty file, and record log, and go on
		cat /dev/null > $statFileName
		md5sum $statFileName > $localStatTempFile".md5"
		log "WARN" "If new kt click stat switch is off, then touch empty file[$localStatTempFile]!"
	fi
	#deal with si keyword click stat ------ end
	
	#deal with si product click stat ------ begin
	statFileName=${SI_PRODUCT_FILE_PRE}${curTime}${FILE_SUF}
	localStatTempFile=$LOCAL_TEMP/$statFileName
	localStatDestFile=$localDestPath/$statFileName
	
	if [ $SI_PRODUCT_CLICK_SWITCH -eq 1 ]
	then
		...
		#if SI_PRODUCT_CLICK_SWITCH is off, then touch empty file, and record log, and go on
		cat /dev/null > $statFileName
		md5sum $statFileName > $localStatTempFile".md5"
		log "WARN" "If new kt click stat switch is off, then touch empty file[$localStatTempFile]!"
	fi
	#deal with si product click stat ------ end
	...
	then
		...
		#if ATSTAT_CLICK_SWITCH is off, then touch empty file, and record log, and go on
		cat /dev/null > $statFileName
		md5sum $statFileName > $localStatTempFile".md5"
	...
	if [ $ATTACHSTAT_CLICK_SWITCH -eq 1 ]
	then
		...
		#if ATTACHSTAT_CLICK_SWITCH is off, then touch empty file, and record log, and go on
		cat /dev/null > $statFileName
		md5sum $statFileName > $localStatTempFile".md5"
	...
	if [ $? -ne 0 ]
	then
		SendMailAndMessage "odp_beidou_click: Fail to mv statfile[$curTime] to dest." $alertType
		return 1
	else
...
function output_filelist()
{
	...
	#echo it click file names into the filelist
    itClickFileName=${ITSTAT_FILE_PRE}${curTime}${FILE_SUF}
    echo ${itClickFileName} >> ${localListDestFile}
	
	#echo dt click file names into the filelist
    dtClickFileName=${DTSTAT_FILE_PRE}${curTime}${FILE_SUF}
    echo ${dtClickFileName} >> ${localListDestFile}
	
	#echo new kt click file names into the filelist
    newktClickFileName=${KEYWORD_FILE_PRE}${curTime}${FILE_SUF}
    echo ${newktClickFileName} >> ${localListDestFile}
    ...
    appClickFileName=${STAT_APP_FILE_PRE}${curTime}${FILE_SUF}
    echo ${appClickFileName} >> ${localListDestFile}
	
	#echo device srchs file names into the filelist
    deviceClickFileName=${STAT_DEVICE_FILE_PRE}${curTime}${FILE_SUF}
    echo ${deviceClickFileName} >> ${localListDestFile}
    
    siKtClickFileName=${SI_KEYWORD_FILE_PRE}${curTime}${FILE_SUF}
    echo ${siKtClickFileName} >> ${localListDestFile}
    
    siProdClickFileName=${SI_PRODUCT_FILE_PRE}${curTime}${FILE_SUF}
    echo ${siProdClickFileName} >> ${localListDestFile}
    
    atClickFileName=${ATSTAT_FILE_PRE}${curTime}${FILE_SUF}
    echo ${atClickFileName} >> ${localListDestFile}
	...
	antiDayFileName=${STAT_ANTI_DAY_FILE_PRE}${curTime}${FILE_SUF}
	if [ -f $antiDayFileName ]
	then
		cat $antiDayFileName >> $fileName
		if [ $? -eq 0 ]
		then 
			echo ${fileName} >> ${localListDestFile}
			md5sum $fileName >${localDestPath}/${fileName}.md5
		else
			rm -rf ${localListDestFile}
			SendMailAndMessage "odp_beidou_click: merge anti day file [$antiDayFileName] failed ." $alertType
			return 1
		fi	
	fi
...
#AESC: check and download stat data within 2 days (that day and the day before)
function check_data_before_and_download()
{
	if [ $FIX_DELAY_REVISE_SWITCH -eq 1 ]
	then
		get_srchs_filelist
		FIX_DELAY=$?
	fi
	...
	do
		...
		scanDate=${scanTime:0:8}
		localDestPath=${LOCAL_DEST}/normal/${scanDate}
		localListDestFile=$localDestPath/$listFileName
		...
		then
			...
			if [ $flag -eq 0 ] #successful
			then
				if [ $scanTime -ne $timeFix ] #not FIX_DELAY hour data
				then
					SendMailAndMessage "odp_download_click: data of $scanTime has been re-downloaded successfully." 1
				fi
			fi
		fi
...
#return numHour to revise FIX_DELAY
function get_srchs_filelist()
{
	local timeFix=`date -d "$FIX_DELAY hour ago" +%Y%m%d%H`
	...
	rm -f filelist.*.normal.0.0
	mkdir -p $TMP_PV_FILE_PATH
	
	while [ $scanTime -le $timeEnding ]
	do
		dateTime=${scanTime:0:8}
		fileListName=filelist.$scanTime.normal.0.0
		remotePvFile=${SRCHS_SERVER_ROOT}/${dateTime}/$fileListName
		
		wget -t $MAX_RETRY -q  ftp://$SRCHS_SERVER_URL/$remotePvFile -O $TMP_PV_FILE_PATH/$fileListName
		
		if [ $? -ne 0 ]
		then
			((numHour++))
			log "WARNING" "FIX_DELAY is resised into $numHour"
			return $numHour
		fi
