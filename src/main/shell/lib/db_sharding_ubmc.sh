...
function runsql_clk_read()
{
	local sql=$1
    local dumpfile=$2
	local mysql_conn="${MYSQL_CLIENT} -B -N -h${BEIDOU_DB_IP_CLK_READ} -P${BEIDOU_DB_PORT_CLK_READ} -u${BEIDOU_DB_USER_CLK_READ} -p${BEIDOU_DB_PASSWORD_CLK_READ} -Dbeidoufinan --default-character-set=utf8 --skip-column-names"
...
{
	local sql=$1
	local mysql_conn="${MYSQL_CLIENT} -B -N -h${BEIDOU_DB_IP_XDB} -P${BEIDOU_DB_PORT_XDB} -u${BEIDOU_DB_USER_XDB} -p${BEIDOU_DB_PASSWORD_XDB} -Dbeidouext --default-character-set=utf8"
	db_execute "${mysql_conn}" "${sql}" "/dev/null"
}
...
function runsql_xdb_read()
{
	local sql=$1
    local dumpfile=$2
	local mysql_conn="${MYSQL_CLIENT} -B -N -h${BEIDOU_DB_IP_XDB_READ} -P${BEIDOU_DB_PORT_XDB_READ} -u${BEIDOU_DB_USER_XDB_READ} -p${BEIDOU_DB_PASSWORD_XDB_READ} -Dbeidouext --default-character-set=utf8 --skip-column-names"
	if [[ 1==$3 ]];then
		mysql_conn="${MYSQL_CLIENT} -B -N -h${BEIDOU_DB_IP_XDB} -P${BEIDOU_DB_PORT_XDB} -u${BEIDOU_DB_USER_XDB} -p${BEIDOU_DB_PASSWORD_XDB} -Dbeidouext --default-character-set=utf8"
	fi
	db_execute "${mysql_conn}" "${sql}" "${dumpfile}"
}
#############################################################################################################################
...
function runsql_re_read()
{
	local sql=$1
    local dumpfile=$2
	local mysql_conn="${MYSQL_CLIENT} -B -N -h${BEIDOU_DB_IP_RE} -P${BEIDOU_DB_PORT_RE} -u${BEIDOU_DB_USER_RE} -p${BEIDOU_DB_PASSWORD_RE} -Dbeidoure --default-character-set=utf8 --skip-column-names"
	db_execute "${mysql_conn}" "${sql}" "${dumpfile}"
}
#############################################################################################################################
...
{
	local sql=$1
	local mysql_conn="${MYSQL_CLIENT} -B -N -h${BEIDOU_DB_IP_CAP} -P${BEIDOU_DB_PORT_CAP} -u${BEIDOU_DB_USER_CAP} -p${BEIDOU_DB_PASSWORD_CAP} -Dbeidoucap --default-character-set=utf8"
	db_execute "${mysql_conn}" "${sql}" "/dev/null"
}
#############################################################################################################################
...
function runsql_random_read()
{
   ...
   local mysql_conn="${MYSQL_CLIENT} -B -N -h${machind} -P${DB_PORT_BD_MAID_READ[$sharding_idx]} -u${DB_USER_BD_MAID_READ} -p${DB_PASSWORD_BD_MAID_READ} -Dbeidoucap --default-character-set=utf8 --skip-column-names"
   db_execute "${mysql_conn}" "${sql}" "${dumpfile}"
}
...
function runsql_single_read()
{
   ...
   local mysql_conn="${MYSQL_CLIENT} -B -N -h${machine_id} -P${DB_PORT_BD_MAID_READ[$sharding_idx]} -u${DB_USER_BD_MAID_READ} -p${DB_PASSWORD_BD_MAID_READ} -Dbeidoucap --default-character-set=utf8 --skip-column-names"
...
function runsql_single()
{
   ...
   local mysql_conn="${MYSQL_CLIENT} -B -N -h${machine_id} -P${DB_PORT_BD_MAID[$sharding_idx]} -u${DB_USER_BD_MAID} -p${DB_PASSWORD_BD_MAID} -Dbeidoucap --default-character-set=utf8 --skip-column-names"
...
function runsql_sharding_read()
{
    ...
    do
		local mysql_conn="${MYSQL_CLIENT} -B -N -h${DB_HOST_BD_MAID_READ[$idx]} -P${DB_PORT_BD_MAID_READ[$idx]} -u${DB_USER_BD_MAID_READ} -p${DB_PASSWORD_BD_MAID_READ} -Dbeidou --default-character-set=utf8 --skip-column-names"
...
#$2:number of table counts in sql if the table is sharded
{
    ...
    for((idx=0;idx<db_count;idx++))
    do
		local mysql_conn="${MYSQL_CLIENT} -B -N -h${DB_HOST_BD_MAID[$idx]} -P${DB_PORT_BD_MAID[$idx]} -u${DB_USER_BD_MAID} -p${DB_PASSWORD_BD_MAID} -Dbeidou --default-character-set=utf8 --skip-column-names"
