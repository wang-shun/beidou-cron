 ...
 * @version 1.2.22
 */
public interface MfcAccountDriverProxy {
	/**
	 * getUserProductBalance($userIds, $products,  $opuid = 0)：获取用户余额
	*		
	* <p>分页逻辑包含在底层实现中,
	...
	* 
	* @param userIds : array, 查询的用户ID数组。每个用户ID均是无符号整型。
	* @param products: array, 查询的产品线ID数组。每个产品线ID均是无符号整型。
	* @param opuid: int ,操作人的ID。默认为0。为0时，财务中心不验证操作人ID是否可管辖查询用户；
	* 非0时，财务中心将验证操作人ID是否可管辖查询用户。出于性能考虑，调用方最好自己验证权限，不传递此参数或者参数设为0.字段类型为无符号整型。
	* @param params Rpc调用时需要传递的参数列表，通常是消息头。
	* 
	* @return 用户的分产品线余额信息
	**/	
	UserProductBalanceResult getUserProductBalance(List<Integer> userIds, List<Integer> products, int opuid, Map<String, String> params);
	...
	* @return 用户的分产品线的投资信息
	**/	
	UserProductBalanceResult getUserProductInvest(List<Integer> userIds, List<Integer> products, int opuid, Map<String, String> params);
	 ...
	 * @return 用户账户信息
	 */
	UserAccountResult getUserAccount(Integer userId, List<Integer> products, String forceMaster, Map<String, String> params);
