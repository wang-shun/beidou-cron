package com.baidu.beidou.accountmove.table.wordexclude;
...
public class WordExcludeImporter implements TableImporter{
...
//	private static final Log logger = LogFactory.getLog(AttachInfoImporter.class);
	...
	public void executeImport(int userId) {
		...
		for (List<String> groupit : groupitList) {
			...
			if (keyMapper.getMappedKey(KeyMapper.WORD_EXCLUDE_ID, oldsmartid, userId) != null) {
				continue;
			}
...
//			groupit.set(StringUtil.getIndex("id", getColumnNameArray()), newGenerateUnitId);
			...
			if (newSmartid != null) {
				keyMapper.addKeyMap(KeyMapper.WORD_EXCLUDE_ID, oldsmartid, newSmartid, Integer.valueOf(oldUserid));
			}
		}
	...
	public String getTableName() {
		return TableSchemaInfo.WORD_EXCLUDE;
	}
