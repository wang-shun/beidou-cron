package com.baidu.beidou.accountmove.table.smartideatemplateelementconf;
...
public class SmartIdeaTemplateElementConfImporter implements TableImporter{
...
//	private static final Log logger = LogFactory.getLog(AttachInfoImporter.class);
	...
	public void executeImport(int userId) {
		...
		for (List<String> groupit : groupitList) {
			...
			if (keyMapper.getMappedKey(KeyMapper.SMARTIDEA_TEMPLATE_ELEMENT_CONF_ID, oldsmartid, userId) != null) {
				continue;
			}
...
//			groupit.set(StringUtil.getIndex("id", getColumnNameArray()), newGenerateUnitId);
			...
			if (newSmartid != null) {
				keyMapper.addKeyMap(KeyMapper.SMARTIDEA_TEMPLATE_ELEMENT_CONF_ID, oldsmartid, newSmartid, Integer.valueOf(oldUserid));
			}
		}
	...
	public String getTableName() {
		return TableSchemaInfo.SMARTIDEA_TEMPLATE_ELEMENT_CONF;
	}
