package com.baidu.beidou.accountmove.table.groupsiteprice;
...
public class GroupSitePriceImporter implements TableImporter{
...
//	private static final Log logger = LogFactory.getLog(AttachInfoImporter.class);
	...
	public void executeImport(int userId) {
		...
		for (List<String> groupit : groupitList) {
			...
			if (keyMapper.getMappedKey(KeyMapper.GROUPSITEPRICE_ID, oldsmartid, userId) != null) {
				continue;
			}
...
//			groupit.set(StringUtil.getIndex("id", getColumnNameArray()), newGenerateUnitId);
			...
			if(StringUtils.isEmpty(groupit.get(StringUtil.getIndex("price", TableSchemaInfo.groupsiteprice)))){
				params[StringUtil.getIndex("price", TableSchemaInfo.groupsiteprice)-1] = null;
			}
			
			String newSmartid = dataAccessService.updateInfo(sql, params, Integer.parseInt(newUserId));
			if (newSmartid != null) {
				keyMapper.addKeyMap(KeyMapper.GROUPSITEPRICE_ID, oldsmartid, newSmartid, Integer.valueOf(oldUserid));
			}
		}
	...
	public String getTableName() {
		return TableSchemaInfo.GROUPSITEPRICE;
	}
