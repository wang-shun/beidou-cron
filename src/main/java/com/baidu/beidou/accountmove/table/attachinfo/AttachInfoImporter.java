package com.baidu.beidou.accountmove.table.attachinfo;
...
public class AttachInfoImporter implements TableImporter{
...
//	private static final Log logger = LogFactory.getLog(AttachInfoImporter.class);
	...
	public void executeImport(int userId) {
		...
		for (List<String> groupit : groupitList) {
			String oldAttachInfoid = groupit.get(StringUtil.getIndex("id", getColumnNameArray()));
			...
			if (keyMapper.getMappedKey(KeyMapper.ATTACH_INFO_ID, oldAttachInfoid, userId) != null) {
				continue;
			}
...
//			groupit.set(StringUtil.getIndex("id", getColumnNameArray()), newGenerateUnitId);
			...
			String newAttachInfoid = dataAccessService.updateInfo(sql, params, Integer.parseInt(newUserId));
			if (newAttachInfoid != null) {
				keyMapper.addKeyMap(KeyMapper.ATTACH_INFO_ID, oldAttachInfoid, newAttachInfoid, Integer.valueOf(oldUserid));
			}
		}
	...
	public String getTableName() {
		return TableSchemaInfo.ATTACH_INFO;
	}
