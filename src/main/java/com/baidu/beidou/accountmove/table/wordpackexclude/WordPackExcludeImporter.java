package com.baidu.beidou.accountmove.table.wordpackexclude;
...
public class WordPackExcludeImporter implements TableImporter{
...
//	private static final Log logger = LogFactory.getLog(AttachInfoImporter.class);
	...
	public void executeImport(int userId) {
		...
		for (List<String> groupit : groupitList) {
			String oldsmartid = groupit.get(StringUtil.getIndex("wpe_id", getColumnNameArray()));
			...
			if (keyMapper.getMappedKey(KeyMapper.WORD_PACK_EXCLUDE_ID, oldsmartid, userId) != null) {
				continue;
			}
...
//			groupit.set(StringUtil.getIndex("id", getColumnNameArray()), newGenerateUnitId);
			...
			if (newSmartid != null) {
				keyMapper.addKeyMap(KeyMapper.WORD_PACK_EXCLUDE_ID, oldsmartid, newSmartid, Integer.valueOf(oldUserid));
			}
		}
	...
	public String getTableName() {
		return TableSchemaInfo.WORD_PACK_EXCLUDE;
	}
