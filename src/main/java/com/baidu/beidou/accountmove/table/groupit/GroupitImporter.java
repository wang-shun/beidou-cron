package com.baidu.beidou.accountmove.table.groupit;
...
public class GroupitImporter implements TableImporter{
	...
	public void executeImport(int userId) {
		...
		for (List<String> groupit : groupitList) {
			...
			if (keyMapper.getMappedKey(KeyMapper.ITID, oldItid, userId) != null) {
				continue;
			}
			...
			long newGenerateItId = idService.generateKeys(Integer.valueOf(newUserId), "cprogroupit", 1);
			...
			if (newGroupId != null) {
				keyMapper.addKeyMap(KeyMapper.ITID, oldItid, newItId, Integer.valueOf(oldUserid));
			} else {
				keyMapper.addKeyMap(KeyMapper.ITID, oldItid, String.valueOf(newGenerateItId),
	...
	public String getTableName() {
		return TableSchemaInfo.GROUPIT;
	}

	public String[] getColumnNameArray() {
		return TableSchemaInfo.cprogroupit;
	}
