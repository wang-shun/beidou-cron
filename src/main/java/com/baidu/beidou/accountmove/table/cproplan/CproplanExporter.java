...
public class CproplanExporter implements TableExporter {
	...
	public void executeExport(int userId) {
		...
		String exportSql = "select "
				+ StringUtil.getStringFromArray(TableSchemaInfo.cproplan, ",")
				+ " from " + TableNameUtil.getActualTableName(getTableName(), userId)
				+ " where userid = " + userId + ";";
