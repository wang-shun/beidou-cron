...
public class GroupSiteFilterImporter implements TableImporter{
...
//	private static final Log logger = LogFactory.getLog(AttachInfoImporter.class);
	...
	public void executeImport(int userId) {
		...
		for (List<String> groupit : groupitList) {
			...
			if (keyMapper.getMappedKey(KeyMapper.GROUPSITEFILTER_ID, oldsmartid, userId) != null) {
				continue;
			}
...
//			groupit.set(StringUtil.getIndex("id", getColumnNameArray()), newGenerateUnitId);
			...
			if (newSmartid != null) {
				keyMapper.addKeyMap(KeyMapper.GROUPSITEFILTER_ID, oldsmartid, newSmartid, Integer.valueOf(oldUserid));
			}
		}
